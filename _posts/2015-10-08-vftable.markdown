...
원래 적고 싶었던건 CRTP(Curiously recurring template pattern)에 대해서 였다. CRTP를 사용하는 이유는 vftable을 이용해서 런타임에 동적으로 동작하는 방식은 성능에 나쁘기 때문에(주소연산이 많이 들어가니까), 템플릿을 이용해서 컴파일타임에 결정하기 위해서다.

사실.. vftable이 어떻게 동작하는지 책에서만 봐서 그동안 "그런가보다.." 하면서 사용해왔는데, 이번기회에 디버깅 좀 해봤다. 덕분에 어셈블리도 조금 알게됬네. 하하하
